...
## スキルセット: ソフトウェア研究開発・セキュリティ・プライバシーテック

専門は分散並列処理・データベース。他、OSなど低レイヤなシステム全般が興味。コンピュータサイエンスに関する分野は一通り押さえている。

2021/09からはトヨタ自動車株式会社にてデータベース・ストリーム・秘密計算などの研究、車両データプライバシー保護施策、新規プラットフォーム事業の企画開発。
...
- セキュリティコンサルティング
  - 社労士事務所に対し [SECURITY ACTION (二つ星)](https://www.ipa.go.jp/security/security-action/sa/index.html) 導入支援
- ソフトウェア研究開発
